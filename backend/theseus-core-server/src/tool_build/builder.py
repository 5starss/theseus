...
class ToolBuilder:
    ...
    def _build_tool_message(*, approved_plan: dict, project_id: int, chat_session_id: int, tool_plan_id: int) -> str:
        return (
            ...
            "- Use run_remote_command(context, command, cwd='.', timeout_seconds=180) instead of direct SSH, "
            "paramiko, subprocess, or os.system.\n"
            "- If Remote Workspace is required but unavailable, return ToolResult(output=<clear error>, is_error=True).\n"
            "- Generated Theseus tools are user-facing project tools; SSH Connector is only an internal execution helper.\n"
