...
class ToolGenerationEventPublisher:
    async def publish(self, key: str, event: BaseModel | dict[str, Any]) -> None:
        raise NotImplementedError


class KafkaToolGenerationEventPublisher(ToolGenerationEventPublisher):
    def __init__(self) -> None:
        self.bootstrap_servers = settings.CORE_KAFKA_BOOTSTRAP_SERVERS
        self.topic = settings.KAFKA_TOPIC_TOOL_GENERATION_EVENT
        self._producer = None
    ...
    async def start(self) -> None:
        ...
        logger.info("Kafka Tool generation compatibility event producer started.")
    ...
    async def stop(self) -> None:
        ...
        self._producer = None
        logger.info("Kafka Tool generation compatibility event producer stopped.")
